...
Verify the update by checking the `karmada-controller-manager` manifest before and after the update:
...
Install or upgrade the chart, specifying the version with the `--version` argument. Replace `<x.x.x>` with your target version:
```bash
