...
- **Alert threshold**: Alert immediately when value changes from 1 to 0
- **Example query**:
  ```promql
...
- **Type**: Gauge
- **Labels**: `member_cluster`
- **Description**: CPU utilization percentage per cluster.
- **Why it matters**: High utilization (&gt;85%) causes scheduling failures.
...
#### cluster_memory_allocated_bytes / cluster_memory_allocatable_bytes

- **Type**: Gauge
- **Labels**: `member_cluster`
- **Description**: Memory utilization percentage per cluster.
- **Why it matters**: Memory exhaustion prevents new pod scheduling.
- **Alert threshold**: &gt;85% warning, &gt;95% critical
- **Example query**:
  ```promql
...
- **Alert threshold**: unschedulable &gt; 0 for &gt;15 minutes
- **Example query**:
  ```promql
...
- **Alert threshold**: &gt;100 or continuously growing
- **Example query**:
  ```promql
...
#### karmada_create_resource_to_cluster / karmada_update_resource_to_cluster / karmada_delete_resource_from_cluster

- **Type**: Counter
- **Labels**: `result`, `apiversion`, `kind`, `member_cluster`
- **Description**: Count of resource operations to member clusters by result.
- **Why it matters**: Errors indicate connectivity or permission issues with member clusters.
- **Alert threshold**: Error rate &gt; 5%
- **Example query**:
  ```promql
...
- **Alert threshold**: Retry rate &gt; 10% of add rate
- **Example query**:
  ```promql
...
- **Type**: Gauge
- **Labels**: `member_cluster`
- **Description**: Percentage of ready nodes per cluster.
- **Why it matters**: Indicates node health within each cluster. A low ratio suggests node health issues that could impact workload capacity.
- **Alert threshold**: &lt;80%
- **Example query**:
  ```promql
...
topk(10, rate(karmada_create_resource_to_cluster{result="error"}[5m]) by (kind))

# Aggregate when details aren't needed
sum by (member_cluster) (rate(karmada_create_resource_to_cluster[5m]))
```
    ...
    rules:
      ...
      - alert: KarmadaPropagationErrors
        expr: |
          sum(rate(karmada_create_resource_to_cluster{result="error"}[5m]))
          + sum(rate(karmada_update_resource_to_cluster{result="error"}[5m]))
          + sum(rate(karmada_delete_resource_from_cluster{result="error"}[5m])) > 1
        for: 5m
        labels:
          severity: critical
        annotations:
...
6. Check resource creation to clusters:
   ```promql
   rate(karmada_create_resource_to_cluster{result="error"}[5m]) by (member_cluster, kind)
   ```
...
sum(rate(karmada_create_resource_to_cluster[5m]))
+ sum(rate(karmada_update_resource_to_cluster[5m]))
+ sum(rate(karmada_delete_resource_from_cluster[5m]))

# Operations by result (success vs error)
sum by (result) (
  rate(karmada_create_resource_to_cluster[5m])
  + rate(karmada_update_resource_to_cluster[5m])
  + rate(karmada_delete_resource_from_cluster[5m])
)
```
