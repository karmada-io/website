...
codegen 包含许多代码自动生成工具，包括：`deepcopy-gen`，`register-gen`，`conversion-gen`，`client-gen`，`lister-gen`，`informer-gen`，`openapi-gen`。请在文件 `hack/update-codegen.sh` 中为新增的 API 配置代码生成器。

## 更新 crdgen

使用 controller-gen 为新增的 API 生成 CRD Yaml 文件，请在文件 `hack/update-crdgen.sh` 中配置。

## 更新 import 别名

为了统一导入 API 包名时的格式，Karmada 在文件 `.import-aliases` 中定义了不同 API 包的导入别名，请检查是否需要修改。

## CRD 安装

为确保正确安装新增的 API，请修改以下文件的配置：
...
如果新增的 API 位于新的 group 中，请修改以下文件并为新的 API 添加相应的配置，以确保正确安装 helm：
...
为了在客户端调用时正确发现新增的 API，请检查是否需要修改以下文件的配置：
