...
污点是触发 Karmada **集群故障迁移的核心机制**。当集群携带了工作负载无法容忍的 `NoExecute` 污点时，Karmada 会驱逐该工作负载并将其重新调度到其他健康集群。本文介绍 Karmada 如何管理集群污点，以及如何使用 `ClusterTaintPolicy` API 配置污点规则。
...
了解集群污点的工作原理后，继续配置集群级故障迁移行为：

- [集群故障迁移](./cluster-failover.md)
