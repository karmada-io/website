...
目前本文档介绍了通过社区维护的 `hack/deploy-karmada.sh` 脚本安装工具部署集群时，集群中的secret内容有关的证书信息，由于目前在 Karmada 系统中存在四种部署方式 (通过 `karmadactl init` 部署，通过 `karmada-operator` 部署，通过 `helm`部署，通过`hack/deploy-karmada.sh` 脚本安装工具部署) ，在`v1.13`及以前的Karmada版本中,Karmada集群中的组件共享相同的证书内容，这将会是一个为了规范和实现Karmada新证书标准的先遣更改 (有关该更改的信息可以在[相关issue](https://github.com/karmada-io/karmada/issues/6091)中找到) 
