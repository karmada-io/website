...
* `MultiClusterService` 支持在 `member1` 和 `member2` 的 `sample-app` Service 之间进行跨集群访问。
* 从 `member1` 请求该服务，流量将路由到两个集群中的后端 Pod，并触发 Pod 自定义指标（`http_requests_total`）的增长。
...
`MultiClusterService` 用于实现 `member1` 和 `member2` 集群中 `sample-app` Service 之间的跨集群访问。当 member1 集群内的客户端访问 `sample-app` 时，请求不再仅局限于 member1 本地的后端 Pod，还可以被路由至 member2 集群中 `sample-app` Service 的后端 Pod。 更多 `MultiClusterService` 的用法，请参考[MultiClusterService 用户指南](../userguide/service/multi-cluster-service-with-native-svc-access.mdx)。
...
部署完成后，从 `member1` 集群向 `sample-app` 发起的请求，可被路由至 `member1` 和 `member2` 两个集群的后端 Pod。
...
* 通过 `hey` 工具从 `member1` 集群访问该服务，以在两个集群上同时拉高 `sample-app` Pod 的自定义指标（`http_requests_total`）。
  ```sh
