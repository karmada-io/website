...
When a member cluster is not ready (for example, when the Karmada control plane cannot access the cluster's kube-apiserver), Karmada does **not** delete the MultiClusterService or EndpointSlice resources for that cluster. This is to avoid service interruption, as pods in the cluster may still be running and serving traffic. However, if a cluster is not ready and its backend pods are also unavailable, multi-cluster service discovery may fail. In this situation, you can modify or delete the MultiClusterService resource to stop using the service on the target cluster.
